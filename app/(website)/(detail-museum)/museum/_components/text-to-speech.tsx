...
export default function TextToSpeech({
    ...
    return (
        <div className="flex items-center border border-yellow text-yellow rounded-full px-4 py-3 shadow-lg w-full max-w-sm mx-auto">
            {/* Play/Pause Button */}
            <button
            ...
            >
                {isLoading ? (
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-yellow"></div>
                ) : isPlaying ? (
                    <Pause size={20} className="[&>*]:stroke-yellow" />
                ) : (
                    <Play size={20} className="[&>*]:stroke-yellow" />
                )}
            </button>

            {/* Progress Bar */}
            <div className="flex-1 relative">
                <div className="h-0.5 border border-yellow rounded-full overflow-hidden">
                    <div
                        className="h-full bg-yellow rounded-full transition-all duration-300 ease-out"
                        style={{ width: `${progressPercentage}%` }}
                    />
                </div>
                {/* Progress Dot */}
                <div
                    className="absolute top-1/2 w-1.5 h-1.5 mt-[3px] bg-yellow rounded-full -translate-y-1/2 transition-all duration-300 ease-out"
            ...
            {/* Error Display */}
            {error && null}
            {/* <div className="absolute top-full left-0 right-0 mt-2 p-2 bg-red-500 text-yellow text-xs rounded">
                {error}
            </div> */}
